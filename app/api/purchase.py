...
from app.models import Purchase, Supplier, Product
...
async def list_purchases(
    ...
    async with async_session() as session:
        ...
        r = await session.execute(select(func.count()).select_from(Purchase).where(and_(*conditions)) if conditions else select(func.count()).select_from(Purchase))
        total = r.scalar() or 0
        ...
        return {
            ...
            "items": [{
                "id": p.id, "po_no": p.po_no or "", "supplier_id": p.supplier_id,
                "supplier_name": p.supplier_name or "", "contact": p.contact or "",
                "phone": p.phone or "", "amount": float(p.amount or 0),
                "paid": float(p.paid or 0), "debt": float(p.debt or 0),
...
async def create_purchase(req: dict = Body(...)):
    async with async_session() as session:
        ...
        seq_r = await session.execute(select(func.count(Purchase.id)).where(Purchase.po_no.like(f"PO{today}%")))
        ...
        p = Purchase(
            ...
            expected_date=datetime.strptime(req.get("expected_date", today), "%Y-%m-%d").date() if req.get("expected_date") else None,
            items=items,
    ...
    async with async_session() as session:
        ...
        if new_status == "已完成":
            ...
            for item in (p.items or []):
                pr = await session.execute(select(Product).where(Product.id == item.get("product_id")))
