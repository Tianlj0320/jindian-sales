...
async def list_suppliers():
    async with async_session() as session:
        result = await session.execute(select(Supplier).order_by(Supplier.code))
        ...
        items = [
            SupplierListItem(
                ...
                phone=s.phone or "",
                delivery_days=s.delivery_days or 7,
                address=s.address or "",
                payment=s.payment or "",
            )
            for s in suppliers
        ]
...
async def create_supplier(req: dict = Body(...)):
    async with async_session() as session:
        # 查最大编号
        seq_result = await session.execute(select(func.count(Supplier.id)))
...
async def update_supplier(supplier_id: int, req: dict = Body(...)):
    async with async_session() as session:
        ...
        if "delivery_days" in req:
            supplier.delivery_days = req["delivery_days"]
        if "address" in req:
            supplier.address = req["address"]
        if "payment" in req:
            supplier.payment = req["payment"]
...
async def list_categories(supplier_id: int | None = Query(None)):
    """布版系列列表（按供应商筛选）"""
    async with async_session() as session:
...
async def create_category(req: dict = Body(...)):
    async with async_session() as session:
        ...
        seq_result = await session.execute(
            select(func.count(FabricCategory.id)).where(FabricCategory.supplier_id == supplier_id)
        )
        seq = (seq_result.scalar() or 0) + 1
...
async def update_category(category_id: int, req: dict = Body(...)):
    async with async_session() as session:
        result = await session.execute(
            select(FabricCategory).where(FabricCategory.id == category_id)
        )
        ...
        if "supplier_id" in req:
            cat.supplier_id = req["supplier_id"]
        await session.commit()
...
async def list_products(
    keyword: str | None = Query(None, description="搜索：名称/编号"),
    ...
    async with async_session() as session:
        ...
        if keyword:
            kw = f"%{keyword}%"
            conditions.append((Product.name.ilike(kw)) | (Product.code.ilike(kw)))
        if supplier_id:
        ...
        items = [
            ProductListItem(
                ...
                supplier_name=sup_map.get(p.supplier_id, type("", (), {"name": ""})()).name
                if p.supplier_id
                else "",
                category_id=p.category_id or 0,
                category_name=cat_map.get(p.category_id, type("", (), {"name": ""})()).name
                if p.category_id
                else "",
    ...
    async with async_session() as session:
        ...
        if p.category_id:
            cr = await session.execute(
                select(FabricCategory).where(FabricCategory.id == p.category_id)
            )
...
async def create_product(req: dict = Body(...)):
    async with async_session() as session:
        ...
        if category_id:
            ...
            cat = cr.scalar_one_or_none()
            if cat:
                cat_code = cat.code[-2:] if cat.code else "00"
